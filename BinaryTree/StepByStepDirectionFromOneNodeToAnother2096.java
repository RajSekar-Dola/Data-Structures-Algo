class Solution {
    public static TreeNode LowestCommnAncestor(TreeNode root, int startValue, int destValue) {
        if (root == null || root.val == startValue || root.val == destValue) {
            return root;
        }

        TreeNode leftChild = LowestCommnAncestor(root.left, startValue, destValue);
        TreeNode rightChild = LowestCommnAncestor(root.right, startValue, destValue);
    ...
    public String getDirections(TreeNode root, int startValue, int destValue) {
        TreeNode LCA = LowestCommnAncestor(root, startValue, destValue);
        StringBuilder res1 = new StringBuilder("");
        
        findPath(LCA,startValue,res1);

        for(int i=0;i<res1.length();i++){
            res1.setCharAt(i,'U');
        }

        findPath(LCA,destValue,res1);
